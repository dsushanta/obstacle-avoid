...
public class DebugCameraConfig {
    ...
    private void init() {
        ...
        if(fileHandle.exists())
            ...
            log.info("Using Default conig as file does not exist : "+FILE_PATH);
            setupDefault();
        }
    }
    ...
    private void load() {
        try {
            ...
            logKey = getInputKeyValue(root, LOG_KEY, DEFAULT_ZOOM_LOG_KEY);


        } catch(Exception e) {
            log.error("Error loading file ... "+FILE_PATH);
    ...
    private void setupDefault() {
        ...
        logKey = DEFAULT_ZOOM_LOG_KEY;
    }

    private static int getInputKeyValue(JsonValue root, String name, int defaultValue) {
        String keyString = root.getString(name, Input.Keys.toString(defaultValue));
